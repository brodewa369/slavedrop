...
impl Window {
  pub(crate) fn new<T>(
    ...
    // Vendor patch (slavedrop): when decorations are enabled, skip tao's
    // hardcoded CSD HeaderBar (decoration_layout "menu:minimize,maximize,close"
    // forces buttons to the right) so the compositor draws the native
    // server-side titlebar instead — KWin/Layan on KDE, matching every other
    // app. Original tao behavior kept for decorations:false windows.
