...
function applySkin() {
  ...
  document.documentElement.style.setProperty('--bg-img', state.bgCustom && state.skin === 'aurora' ? `url("file://${state.bgCustom}")` : 'none');
  reskinDom();
  renderBgRow();
  syncHeaderMenu();
}
...
function renderCalendar() {
  ...
  for (let d = 1; d <= dim; d++) {
    ...
    cells += `<div class="cal-cell ${ds === todayISO ? 'today' : ''}">
      <span class="dnum">${d}</span>
      ${shown.map(x => `<span class="dl ${x.done ? 'done' : ''}" data-open="${x.pid}" title="${esc(x.pname)}: ${esc(x.text)}">${esc(x.pname)}</span>`).join('')}
      ${items.length > 2 ? `<span class="dl more">+${items.length - 2} more</span>` : ''}
    </div>`;
  }
  $('#cal').innerHTML = DOW.map(d => `<div class="cal-dow">${d}</div>`).join('') + cells;

  const list = state.deadlines.filter(d => d.deadline >= iso(new Date(cur.getFullYear(), cur.getMonth(), 1)) && d.deadline <= iso(new Date(cur.getFullYear(), cur.getMonth() + 1, 0)));
  $('#cal-list').innerHTML = `<h4>${list.length} deadline${list.length === 1 ? '' : 's'} this month</h4>` +
    (list.length ? list.map(d => `<div class="dl-row ${!d.done && d.deadline < todayISO ? 'overdue' : ''}" data-open="${d.pid}">
      <span class="when">${esc(d.deadline)}</span>
      <span class="who">${esc(d.pname)}</span>
      <span class="what">${esc(d.text)}${d.done ? ' (done)' : ''}</span>
      <span class="badge">${esc(new Date(d.deadline + 'T00:00:00').toLocaleDateString(undefined, { weekday: 'long' }))}</span>
    </div>`).join('') : '<p style="color:var(--fg-3);font-size:13px;margin:0">No deadlines this month. Add a task with a deadline in any project.</p>');
...
function renderStaticLabels() {
  ...
  $('#btn-add').innerHTML = skinIcons(`<i class="ph ph-plus"></i> ${t('newProject')}`);
