...
const state = {
  ...
  detailId: null,
  editing: null,
  selectedDate: null,  // toggle: selected date in calendar for deadline detail
...
function applySkin() {
  ...
  document.documentElement.style.setProperty('--bg-img', state.bgCustom && state.skin === 'aurora' ? `url("${window.__TAURI__.core.convertFileSrc(state.bgCustom)}")` : 'none');
  reskinDom();
  renderBgRow();
  syncHeaderMenu();
}
...
function renderDaily(stats) {
  if (!stats) return;
  // daily progress is driven by checked tasks, not just the project done flag,
  // so a fully checked project counts as done even if the flag was never set
  const doneCount = state.projects.filter(p => allTasksDone(p)).length;
  const pct = stats.total ? Math.round((doneCount / stats.total) * 100) : 0;
  const dailyEl = $('#header-daily');
  if (!dailyEl) return;
  dailyEl.innerHTML = `
    <span class="d-label">${t('dailyProgress')}</span>
    <span class="d-counts">${stats.total} ${t('projects')} · ${doneCount} ${t('done')}</span>
...
function renderFilterDropdowns(stats) {
  ...
  for (const dd of FILTER_DD) {
    ...
    btn.innerHTML = skinIcons(`${cur ? esc(dd.fmt(cur)) : esc(dd.allLabel())} <i class="ph ph-caret-down" style="font-size:11px"></i>`);
...
// active-filter indicator on the topbar filter icon (dot + highlight)
function syncFilterIndicator() {
  const active = !!(state.filter.network || state.filter.category || state.filter.rank || state.filter.todoOnly);
  ...
  const sw = $('#filter-todo');
  if (sw) sw.setAttribute('aria-checked', String(!!state.filter.todoOnly));
}

// ---------- icons ----------
function iconHtml(p, big = false) {
  if (p.icon) {
    return `<img class="picon" src="${window.__TAURI__.core.convertFileSrc(esc(p.icon).replace(/"/g, ''))}" alt="" onerror="this.style.display='none'">`;
  }
...
document.addEventListener('click', () => {
  ...
  const fp = $('#filter-pop');
  if (fp && !fp.hidden) { fp.hidden = true; const fb = $('#btn-filter'); if (fb) fb.setAttribute('aria-expanded', 'false'); }
});
...
function dlBadge(t) {
  ...
  const dp = t.deadline.slice(0, 10);  // date part (deadline may carry T HH:MM)
  const overdue = !t.done && dp < todayISO;
  const soon = !t.done && !overdue && dp <= iso(new Date(TODAY.getTime() + 3 * 864e5));
  return `<span class="dl-badge ${overdue ? 'overdue' : soon ? 'soon' : ''}"><i class="ph ph-clock"></i> Deadline ${esc(t.deadline.replace('T', ' '))}</span>`;
}
...
async function cardAction(act, p) {
  ...
  if (act === 'del') {
    if (!(await window.api.confirmDialog(t('deleteConfirm', p.name)))) return;
...
// ---------- custom deadline picker (native input[type=date] popup lags/sticks in WebKitGTK) ----------
const dlSplit = (v) => { const i = String(v || '').indexOf('T'); return i < 0 ? { date: v || '', time: '' } : { date: v.slice(0, i), time: v.slice(i + 1) }; };
const dlJoin = (date, time) => (date ? (time ? `${date}T${time}` : date) : '');
function dlSanTime(v) {
  v = String(v || '').replace(/[^0-9:]/g, '');
  if (!v.includes(':') && v.length >= 3) v = v.slice(0, 2) + ':' + v.slice(2);
  const p = v.split(':');
  if (p.length > 2) v = p[0] + ':' + p.slice(1).join('');
  const h = (v.split(':')[0] || '').slice(0, 2), m = (v.split(':')[1] || '').slice(0, 2);
  return m.length ? `${h}:${m}` : h;
}
let DLPOP = null, dlTarget = null, dlRow = null, dlCursor = null;
function ensureDlPop() {
  if (DLPOP) return DLPOP;
  DLPOP = document.createElement('div');
  DLPOP.id = 'dl-pop'; DLPOP.className = 'dl-pop'; DLPOP.hidden = true;
  document.body.appendChild(DLPOP);
  document.addEventListener('mousedown', (e) => {
    if (!DLPOP || DLPOP.hidden) return;
    if (DLPOP.contains(e.target) || (dlTarget && dlTarget.contains(e.target))) return;
    closeDlPop();
  });
  document.addEventListener('keydown', (e) => { if (e.key === 'Escape') closeDlPop(); });
  return DLPOP;
}
function closeDlPop() { if (DLPOP) DLPOP.hidden = true; dlTarget = null; dlRow = null; }
function openDlPop(input, rowObj) {
  const pop = ensureDlPop();
  dlTarget = input; dlRow = rowObj || null;
  const cur = dlSplit(rowObj ? rowObj.deadline : input.value);
  dlCursor = new Date((cur.date || iso(TODAY)) + 'T00:00:00');
  if (isNaN(dlCursor)) dlCursor = new Date(TODAY);
  dlCursor.setDate(1);
  pop.hidden = false;
  pop.style.top = '0px'; pop.style.left = '0px'; pop.style.visibility = 'hidden';
  drawDlPop();
  const r = input.getBoundingClientRect(), pr = pop.getBoundingClientRect();
  ...
  pop.style.top = top + 'px'; pop.style.left = left + 'px'; pop.style.visibility = '';
}
function drawDlPop() {
  const pop = DLPOP;
  const cur = dlSplit(dlRow ? dlRow.deadline : (dlTarget ? dlTarget.value : ''));
  const y = dlCursor.getFullYear(), m = dlCursor.getMonth();
  const pad = new Date(y, m, 1).getDay(), dim = new Date(y, m + 1, 0).getDate();
  let cells = '';
  for (let i = 0; i < pad; i++) cells += '<div class="dl-cell pad"></div>';
  for (let d = 1; d <= dim; d++) {
    const ds = `${y}-${String(m + 1).padStart(2, '0')}-${String(d).padStart(2, '0')}`;
    cells += `<div class="dl-cell${ds === cur.date ? ' sel' : ''}" data-ds="${ds}">${d}</div>`;
  }
  pop.innerHTML = `<div class="dl-pop-head">
      <button type="button" class="dl-nav" data-nav="-1" title="Previous month"><i class="ph ph-caret-left"></i></button>
      <span class="t">${MON[m]} ${y}</span>
      <button type="button" class="dl-nav" data-nav="1" title="Next month"><i class="ph ph-caret-right"></i></button>
      <button type="button" class="dl-close" title="Close"><i class="ph ph-x"></i></button>
    </div>
    <div class="dl-grid">${DOW.map(d => `<div class="dow">${d.slice(0, 3)}</div>`).join('')}${cells}</div>
    <div class="dl-pop-foot"><span>Time</span><span style="text-transform:none;font-weight:400;color:var(--fg-2)">set in the HH:MM field beside the date</span></div>`;
  pop.querySelector('.dl-close').onclick = closeDlPop;
  pop.querySelectorAll('.dl-nav').forEach(b => b.onclick = () => { dlCursor.setMonth(dlCursor.getMonth() + Number(b.dataset.nav)); drawDlPop(); });
  pop.querySelectorAll('.dl-cell[data-ds]').forEach(c => c.onclick = () => {
    const ds = c.dataset.ds;
    if (dlRow) dlRow.deadline = dlJoin(ds, dlSplit(dlRow.deadline).time);
    if (dlTarget) dlTarget.value = ds;
    drawDlPop();  // stays open: pick date, then set time — close via ✕ / outside click / Esc
  });
}

// countdown helpers (deadline detail panel, live)
function dlEndMs(s) {
  if (!s) return 0;
  if (s.length > 10) { const d = new Date(s); if (!isNaN(d)) return d.getTime(); }
  const d2 = new Date(s + 'T23:59:59');  // date-only deadline = end of that day
  return isNaN(d2) ? 0 : d2.getTime();
}
function fmtRemain(ms) {
  const parts = (tot) => {
    const s = Math.max(0, Math.floor(tot / 1000));
    return { d: Math.floor(s / 86400), h: Math.floor(s / 3600) % 24, m: Math.floor(s / 60) % 60, s: s % 60 };
  };
  if (ms <= 0) {
    const { d, h, m, s } = parts(-ms);
    const body = d > 0 ? `${d}d ${h}h ${m}m ${s}s` : h > 0 ? `${h}h ${m}m ${s}s` : `${m}m ${s}s`;
...
function tickCountdowns() {
  const now = Date.now();
  let any = false;
  $$('#cal-dl-detail .cd[data-dl], #cal-dl-summary .cd[data-dl]').forEach(el => {
    any = true;
    const ms = dlEndMs(el.dataset.dl) - now;
    const txt = fmtRemain(ms);
    if (el.textContent !== txt) el.textContent = txt;
    el.classList.toggle('over', ms <= 0);
  });
  const clk = document.getElementById('dl-clock');
  if (clk) {
    const n = new Date();
    const p = (x) => String(x).padStart(2, '0');
    clk.textContent = `${p(n.getHours())}:${p(n.getMinutes())}:${p(n.getSeconds())}`;
  }
  if (!any && !clk) stopCountdown();  // panel gone -> cleanup interval
}
function startCountdown() { stopCountdown(); CD_TIMER = setInterval(tickCountdowns, 1000); tickCountdowns(); }
...
function renderCalendar() {
  ...
  // MASTER collapse toggle (persisted in localStorage): hides the day grid AND the weekly list in sync
  ...
  tBtn.title = state.calCollapsed ? 'Expand calendar & deadlines' : 'Collapse calendar & deadlines';
  ...
  const byDate = {};
  for (const d of state.deadlines) (byDate[d.deadline.slice(0, 10)] ||= []).push(d);
  ...
  for (let d = 1; d <= dim; d++) {
    ...
    const isSelected = state.selectedDate === ds;
    cells += `<div class="cal-cell ${ds === todayISO ? 'today' : ''}${isSelected ? ' selected' : ''}" data-date="${ds}">
      <span class="dnum">${d}</span>
      ${hasDot ? '<span class="dot"></span>' : ''}
    </div>`;
  }
  $('#cal').innerHTML = DOW.map(d => `<div class="cal-dow">${d.slice(0, 3)}</div>`).join('') + cells;

  // clickable dates: toggle selectedDate for deadline detail
  $$('.cal-cell[data-date]').forEach(el => {
    el.onclick = () => {
  ...
  // weekly deadline summary
  renderWeeklySummary(cur);

  // deadline detail panel
  renderDlDetail();

  // clear old monthly list container (no longer used — weekly summary replaces it)
  const calList = $('#cal-list');
  if (calList) calList.innerHTML = '';
}

// weekly deadline summary: shows deadlines in current week (Sun-Sat)
function renderWeeklySummary(cur) {
  const title = $('#wk-title');
  const list = $('#wk-list');
  if (!title || !list) return;
  // calculate week boundaries: find Sunday of current week
  ...
  const weekDeadlines = state.deadlines.filter(d => {
    const dt = new Date(d.deadline.length > 10 ? d.deadline : d.deadline + 'T00:00:00');
    return dt >= weekStart && dt <= weekEnd;
  });

  if (!weekDeadlines.length) {
    title.textContent = 'Deadlines This Week';
    list.innerHTML = `<p class="empty">No deadlines this week.</p>`;
    return;
  }
  title.textContent = `${weekDeadlines.length} Deadline${weekDeadlines.length === 1 ? '' : 's'} This Week`;
  list.innerHTML = weekDeadlines.map(d => `<div class="dl-item ${!d.done && d.deadline.slice(0, 10) < todayISO ? 'overdue' : ''}" data-open="${d.pid}">
      <span class="when">${esc(d.deadline.replace('T', ' '))}</span>
      <span class="who">${esc(d.pname)}</span>
      <span class="what">${esc(d.text)}</span>
      ${d.done ? '' : `<span class="cd" data-dl="${esc(d.deadline)}"></span>`}
    </div>`).join('');
  $$('#wk-list .dl-item').forEach(el => el.onclick = () => openDetail(Number(el.dataset.open)));
}

// deadline detail panel: toggled by clicking a date with a dot
function renderDlDetail() {
  const detail = $('#cal-dl-detail');
  if (!detail) return;
  if (!state.selectedDate) {
    detail.hidden = true;
    detail.innerHTML = '';
    return;  // ticker keeps running: top-bar clock is always mounted
  }
  const items = state.deadlines.filter(d => d.deadline.slice(0, 10) === state.selectedDate);
  if (!items.length) {
    detail.hidden = true;
    detail.innerHTML = '';
    return;  // ticker keeps running: top-bar clock is always mounted
  ...
  const dateLabel = dt.toLocaleDateString(state.locale === 'id' ? 'id-ID' : 'en-US',
    { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' });
  detail.innerHTML = `<h4>${dateLabel}</h4>` + items.map(d => `
    <div class="dl-item ${!d.done && d.deadline.slice(0, 10) < todayISO ? 'overdue' : ''}" data-open="${d.pid}">
      <span class="when">${esc(d.deadline.replace('T', ' '))}</span>
      <span class="who">${esc(d.pname)}</span>
      <span class="what">${esc(d.text)}${d.done ? ' (done)' : ''}</span>
      ${d.done ? '' : `<span class="cd" data-dl="${esc(d.deadline)}"></span>`}
    </div>`).join('');
  $$('#cal-dl-detail .dl-item').forEach(el => el.onclick = () => openDetail(Number(el.dataset.open)));
  startCountdown();
}
...
window.openDrawer = async function (id = null) {
  ...
  $('#scrim').classList.add('on');
  wireDrawer(p, isNew); setTimeout(() => hoverResync(d), 320);
};
...
function drawerHtml(p, isNew) {
  ...
  return `
    ...
    <div class="drawer-body">
      ...
      <div class="field"><span>${t('network')}</span>
      ...
      <div class="f-row">
        <div class="field"><span>${t('rank')}</span>
      ...
      <div class="f-row">
        <div class="field"><span>${t('cost')}</span>
        ...
        <div class="field"><span>${t('platform')}</span>
      ...
      <div class="field"><span>${t('walletApp')}</span>
...
// WebKit caches :hover while content moves under a STATIONARY cursor (fade-in,
// body scroll, chip insert/delete) -> stale/ghost highlight on the wrong option.
// Flipping pointer-events invalidates that state; next mousemove re-hit-tests clean.
function hoverResync(el) {
  el = el || document.getElementById('drawer');
  if (!el || !el.classList.contains('on')) return;
  el.style.pointerEvents = 'none';
  requestAnimationFrame(() => requestAnimationFrame(() => { el.style.pointerEvents = ''; }));
}
function wireDrawer(p, isNew) {
  const __sb = document.querySelector('#drawer .drawer-body');
  if (__sb) { let __st = 0; __sb.addEventListener('scroll', () => { clearTimeout(__st); __st = setTimeout(() => hoverResync(document.getElementById('drawer')), 140); }, { passive: true }); }
  ...
  const drawPreview = () => { $('#d-icon-preview').innerHTML = iconPath
    ? `<img class="picon" src="${window.__TAURI__.core.convertFileSrc(esc(iconPath))}" alt="">`
  ...
  for (const kind in HOST) {
    ...
    $$('.chip-x', host).forEach(x => x.onclick = async (e) => {
      ...
      if (used > 0 && !(await window.api.confirmDialog(t('confirmDeleteOption', v, used)))) return;
    ...
    if (addBtn) addBtn.onclick = async () => {
      ...
      host.appendChild(chip); hoverResync(document.getElementById('drawer'));
  ...
  const rowHtml = {
    ...
    tasks: (r, i) => `<input class="input" data-k="text" data-i="${i}" placeholder="Task description" value="${esc(r.text)}" style="flex:2.2">
      <input class="input task-deadline" data-i="${i}" data-dldate="${i}" readonly value="${esc(dlSplit(r.deadline).date)}" title="Deadline date — click to open calendar">
      <input class="input task-dltime" data-i="${i}" data-dltime="${i}" maxlength="5" placeholder="14:30" value="${esc(dlSplit(r.deadline).time)}" title="Deadline time (24h HH:MM, optional)">
  ...
  const drawAll = () => {
    for (const key in containers) {
      ...
      if (dlBtn) dlBtn.onclick = () => {
        ...
        rows.tasks.push(t);
        drawAll();
        // open the custom picker right away (native date popup is laggy in WebKitGTK)
        const inputs = $$('#m-tasks .task-deadline');
        if (inputs.length) openDlPop(inputs[inputs.length - 1], rows.tasks[rows.tasks.length - 1]);
      };
      $$('[data-i]', host).forEach(el => {
        const i = Number(el.dataset.i);
        const k = el.dataset.k;
        if (el.dataset.dldate !== undefined) {
          el.onclick = () => openDlPop(el, rows[key][i]);
        } else if (el.dataset.dltime !== undefined) {
          el.oninput = () => {
            rows[key][i].deadline = dlJoin(dlSplit(rows[key][i].deadline).date || iso(TODAY), dlSanTime(el.value));
          };
          el.onblur = () => {
            const v = dlSanTime(el.value);
            el.value = /^([01]?\d|2[0-3]):[0-5]\d$/.test(v) ? v : '';
            rows[key][i].deadline = dlJoin(dlSplit(rows[key][i].deadline).date || '', el.value);
            if (!dlSplit(rows[key][i].deadline).date) rows[key][i].deadline = '';
          };
        } else if (el.type === 'checkbox') el.onchange = () => { rows[key][i][k] = el.checked; };
        else el.oninput = () => {
...
// filter icon: open/close the floating panel (backdrop click closes it via the global handler)
$('#btn-filter').onclick = (e) => {
  e.stopPropagation();
  const fp = $('#filter-pop');
  fp.hidden = !fp.hidden;
  $('#btn-filter').setAttribute('aria-expanded', String(!fp.hidden));
};
$('#filter-pop').onclick = (e) => e.stopPropagation();
...
try { const c = localStorage.getItem('airdrop.calCollapsed'); state.calCollapsed = c === '1'; } catch { /* no storage */ }
...
// self-heal: WebKit can leave aspect-ratio cells stale after a window resize (cells only
// fix themselves when the grid is rebuilt). Rebuild ONCE when resize settles (debounced),
// so no interaction is needed. During the drag only CSS recalcs -> no per-frame JS cost.
let CAL_RS_T = 0;
window.addEventListener('resize', () => {
  clearTimeout(CAL_RS_T);
  CAL_RS_T = setTimeout(() => {
    try { renderCalendar(); } catch { /* DOM not ready */ }
  }, 120);
});
...
function renderStaticLabels() {
  ...
  // search box is icon-only: no placeholder (by design)
  $('#filter-todo-label').textContent = t('showPending');
  ...
  // top-bar clock: ensure mounted (static HTML normally provides it), keep the ticker alive
  if (!document.getElementById('dl-clock')) { const tp = document.getElementById('cal-today'); if (tp) { const c = document.createElement('span'); c.className = 'cal-clock'; c.id = 'dl-clock'; tp.insertAdjacentElement('afterend', c); } }
  startCountdown();
...
// ---------- QA hook: auto-open panels via location.hash (#detail=ID, #drawer=new) ----------
(function hdwHook() {
  if (location.hash !== '#hdw') return;
  setTimeout(() => {
    const q = (s) => document.querySelector(s);
    const head = q('.cal-head');
    const hd = q('.hd-right');
    const title = q('#cal-title');
    const pill = q('#cal-today');
    const clock = q('#dl-clock');
    const prev = q('#cal-prev');
    const next = q('#cal-next');
    const main = q('.cal-main');
    const wkh = q('.wk-head');
    const summ = q('.cal-dl-summary');
    ...
    const lines = [
      ...
      'cal-main = ' + w(main),
      'cal-head = ' + w(head) + ' clientW=' + (head ? head.clientWidth : -1) + ' scrollW=' + (head ? head.scrollWidth : -1),
      'display=' + (cs ? cs.display : '?') + ' wrap=' + (cs ? cs.flexWrap : '?') + ' gap=' + (cs ? cs.gap : '?'),
      'cal-prev  = ' + w(prev),
      'cal-title = ' + w(title) + ' scrollW=' + (title ? title.scrollWidth : -1),
      'cal-next  = ' + w(next),
      'hd-right  = ' + w(hd) + ' scrollW=' + (hd ? hd.scrollWidth : -1),
      '  pill    = ' + w(pill),
      '  clock   = ' + w(clock),
      'SUM all4 = ' + (w(prev) + w(title) + w(next) + w(hd)),
      'sameLine = ' + (title && hd ? (Math.round(title.getBoundingClientRect().top) === Math.round(hd.getBoundingClientRect().top)) : '?'),
      '--- header parity ---',
      'cal-head: top=' + (head ? head.getBoundingClientRect().top.toFixed(1) : '?') + ' h=' + (head ? head.getBoundingClientRect().height.toFixed(1) : '?'),
      'wk-head : top=' + (wkh ? wkh.getBoundingClientRect().top.toFixed(1) : '?') + ' h=' + (wkh ? wkh.getBoundingClientRect().height.toFixed(1) : '?'),
      'deltaTop = ' + (head && wkh ? (wkh.getBoundingClientRect().top - head.getBoundingClientRect().top).toFixed(1) : '?') + '  deltaH = ' + (head && wkh ? (wkh.getBoundingClientRect().height - head.getBoundingClientRect().height).toFixed(1) : '?'),
      'summary margin-top=' + (summ ? getComputedStyle(summ).marginTop : '?') + ' padding-top=' + (summ ? getComputedStyle(summ).paddingTop : '?'),
      'wk-title h4 margin=' + (wkh ? getComputedStyle(wkh.querySelector('h4')).margin : '?'),
    ];
    const box = document.createElement('div');
    box.style.cssText = 'position:fixed;left:12px;top:120px;z-index:99999;background:#000;color:#0f0;font:12px/1.45 monospace;padding:8px 10px;border:1px solid #0f0;white-space:pre;max-width:640px;';
...
(function qaHook() {
  ...
  const dm = h.match(/^#detail=(\d+)/);
  if (dm) { setTimeout(() => openDetail(Number(dm[1])), 300); return; }
  if (h === '#drawer=new') { setTimeout(() => openDrawer(null), 300); return; }
  if (h === '#filterpop') { setTimeout(() => { $('#filter-pop').hidden = false; $('#btn-filter').setAttribute('aria-expanded', 'true'); }, 1400); return; }
  if (h === '#dlpick') { setTimeout(() => openDrawer(null), 300); setTimeout(dlPickSeq, 1000); return; }
  if (h === '#cdtest') { setTimeout(cdProbe, 1200); return; }
  if (h === '#cddelete') { setTimeout(cdProbeDelete, 800); return; }
  if (h === '#cddemo') { setTimeout(cdDemo, 1000); return; }
  if (h === '#expand') {
    setTimeout(() => {
      document.querySelectorAll('.qa-ov').forEach(n => n.remove());
      if (state.calCollapsed) {
        state.calCollapsed = false;
        try { localStorage.setItem('airdrop.calCollapsed', '0'); } catch {}
        renderCalendar();
      }
    }, 400);
    return;
  }
  if (h === '#cellqa') { setTimeout(cellQa, 1000); return; }
  if (h === '#resizetest') { setTimeout(resizeSelfTest, 900); return; }
  if (h === '#phantom') { setTimeout(phantomQa, 700); return; }
  ...
  if (h === '#cdweekly') {
    setTimeout(async () => {
      await cdDemo();
      setTimeout(() => {
        const sum = document.getElementById('cal-dl-summary');
        const det = document.getElementById('cal-dl-detail');
        const ov = document.createElement('div');
        ov.style.cssText = 'position:fixed;top:70px;left:24px;z-index:999;background:#000;color:#3f6;font:12px/1.5 monospace;padding:10px 14px;max-width:820px;white-space:pre-wrap;border:2px solid #3f6;';
        const rows = sum ? [...sum.querySelectorAll('.dl-item')] : [];
        ov.textContent = [
          'CD WEEKLY DUMP',
          'summary rows  : ' + rows.length,
          'cd in summary : ' + (sum ? sum.querySelectorAll('.cd[data-dl]').length : -1),
          ...rows.map((r, i) => `row${i} cd: ` + (r.querySelector('.cd') ? JSON.stringify({ text: r.querySelector('.cd').textContent, dataDl: r.querySelector('.cd').dataset.dl }) : 'NONE')),
          'detail cd     : ' + (det ? det.querySelectorAll('.cd[data-dl]').length : -1),
          'top clock     : ' + (document.getElementById('dl-clock') || {}).textContent,
          'setIntervals   : 1 (global ticker)',
        ].join('\n');
  ...
  if (h === '#cddump') {
    setTimeout(async () => {
      ...
      setTimeout(() => {
        const det = document.getElementById('cal-dl-detail');
        const html = det ? det.innerHTML : 'NO DETAIL ELEMENT';
        const cdCount = det ? det.querySelectorAll('.cd[data-dl]').length : -1;
        const clk = document.getElementById('dl-clock');
        const cd0 = det ? det.querySelector('.cd') : null;
        const ov = document.createElement('div');
        ov.style.cssText = 'position:fixed;top:60px;left:20px;z-index:999;background:#000;color:#3f6;font:12px/1.5 monospace;padding:10px 14px;max-width:760px;white-space:pre-wrap;border:2px solid #3f6;';
        ov.textContent = [
          'CD DUMP',
          'detail-el : ' + (det ? 'yes' : 'NO'),
          'cd spans  : ' + cdCount,
          'cd[0]     : ' + (cd0 ? JSON.stringify({ text: cd0.textContent, dataDl: cd0.dataset.dl, rect: cd0.getBoundingClientRect().toJSON() }) : 'NONE'),
          'clock     : ' + (clk ? JSON.stringify({ text: clk.textContent, rect: clk.getBoundingClientRect().toJSON() }) : 'NONE'),
          'h4        : ' + (det && det.querySelector('h4') ? JSON.stringify(det.querySelector('h4').textContent) : 'NONE'),
          'detail rect: ' + (det ? JSON.stringify(det.getBoundingClientRect().toJSON()) : 'NONE'),
          'innerHTML head: ' + html.slice(0, 300),
        ].join('\n');
        document.body.appendChild(ov);
        toast('CD dump done');
      }, 1200);
  ...
  if (h === '#fontmetric') { setTimeout(fontMetric, 1400); return; }
  if (h === '#compact') { setTimeout(() => { const b = $('#filter-compact'); if (b && !state.compact) b.click(); }, 1400); return; }
  if (h === '#taginspect') { setTimeout(inspectTag, 1400); return; }
  if (h === '#tagmeasure') { setTimeout(measureTagCentering, 1400); return; }
  if (h === '#filterclose') { setTimeout(() => { $('#filter-pop').hidden = false; }, 1400); setTimeout(() => { document.body.click(); }, 2300); return; }
  if (h === '#filteractive') { setTimeout(() => {
    state.filter.network = (state.options.networks && state.options.networks[0]) || 'Solana';
    state.filter.todoOnly = true;
    $('#filter-todo').classList.add('on');
    renderGrid(); syncFilterIndicator();
  }, 1400); return; }
})();

// QA: real-data countdown demo — purge probe project, select earliest live deadline
async function cdDemo() {
  try {
    await cdProbeDelete();
    await refresh();
    const live = (state.deadlines || []).filter(d => !d.done)
      .sort((a, b) => (a.deadline < b.deadline ? -1 : 1));
    state.selectedDate = live.length ? live[0].deadline.slice(0, 10) : todayISO;
    renderCalendar();
    toast('CD demo date: ' + state.selectedDate);
  } catch (e) { toast('CD demo failed: ' + e, true); }
}

// QA: picker timing + close-mechanics probe (Bug 1 acceptance)
function dlPickSeq() {
  const dlBtn = document.querySelector('[data-add-dl]');
  if (dlBtn) dlBtn.click();
  setTimeout(() => {
    const inp = document.querySelector('#m-tasks .task-deadline');
    if (!inp) { toast('QA: no deadline input', true); return; }
    inp.click();  // exercise the REAL wiring (data-i -> openDlPop), not a direct call
    const wired = (DLPOP && !DLPOP.hidden) ? 'OK' : 'FAIL';
    if (!DLPOP || DLPOP.hidden) openDlPop(inp, null);
    const cells = [...DLPOP.querySelectorAll('.dl-cell[data-ds]')].slice(0, 6);
    const times = [];
    for (const c of cells) { const t0 = performance.now(); c.click(); times.push(performance.now() - t0); }
    const close = (how) => {
      if (how === 'outside') document.dispatchEvent(new MouseEvent('mousedown', { bubbles: true }));
      else if (how === 'x') DLPOP.querySelector('.dl-close').click();
      else if (how === 'esc') document.dispatchEvent(new KeyboardEvent('keydown', { key: 'Escape', bubbles: true }));
      return DLPOP.hidden ? 'OK' : 'FAIL';
    };
    const outside = close('outside');
    openDlPop(inp, null); const xok = close('x');
    openDlPop(inp, null); const escok = close('esc');
    openDlPop(inp, null);  // leave open for the visual screenshot
    const ov = document.createElement('div');
    ov.style.cssText = 'position:fixed;left:8px;top:52px;z-index:99999;background:#000;color:#39ff14;border:2px solid #39ff14;padding:8px 10px;font:700 13px monospace;white-space:pre;line-height:1.55;';
    ov.textContent = [
      'DL PICKER QA (custom popup, native removed)',
      'wired-click : ' + wired,
      'pick click ms: ' + times.map(x => x.toFixed(1)).join(', '),
      'max pick ms  : ' + Math.max(...times).toFixed(1),
...
// QA: probe project with +5min and -2h deadlines -> live countdown screenshot
async function cdProbe() {
  try {
    const now = new Date();
    const ds = iso(now);
    const hhmm = (d) => `${String(d.getHours()).padStart(2, '0')}:${String(d.getMinutes()).padStart(2, '0')}`;
    const soon = new Date(now.getTime() + 5 * 60000);
    const past = new Date(now.getTime() - 2 * 3600000);
    const saved = await window.api.saveProject({
      id: null, name: 'ZZ countdown test', website: '', x_account: '',
      category: [], network: [], note: '', icon: '', done: false,
      rank: '', cost: '', status: '', platform: [], wallet_app: '',
      links: [], wallets: [], x_accounts: [], emails: [],
      tasks: [
        { text: 'probe in 5 minutes', deadline: `${ds}T${hhmm(soon)}`, done: false, url: '' },
        { text: 'probe overdue', deadline: `${iso(past)}T${hhmm(past)}`, done: false, url: '' },
      ],
    });
    await refresh();
    state.selectedDate = ds;
    renderCalendar();
    if (saved) window.__cdtest_id = saved.id;
    toast('CD probe ready');
  } catch (e) { toast('CD probe failed: ' + e, true); }
}
async function cdProbeDelete() {
  try {
    const list = await window.api.projects();
    const hits = (list || []).filter(p => p.name === 'ZZ countdown test');
    for (const p of hits) await window.api.deleteProject(p.id);
    await refresh();
    toast('CD probe deleted: ' + hits.length);
  } catch (e) { toast('CD delete failed: ' + e, true); }
}

// QA: open ADD drawer with NO mouse interaction -> who is :hover? which chips look painted?
function phantomQa() {
  try {
    window.openDrawer(null);
    setTimeout(() => {
      const chipsOf = () => Array.from(document.querySelectorAll('#drawer .cat-chip'));
      const hoverLabels = () => chipsOf().filter(c => c.matches(':hover')).map(c => (c.textContent || '').trim().slice(0, 14));
      const chain = Array.from(document.querySelectorAll(':hover')).map(e => {
      ...
      const h1 = hoverLabels();
      const lines = chips.slice(0, 14).map(c => {
        const st = getComputedStyle(c);
        return (c.matches(':hover') ? 'HOVER ' : (c.classList.contains('on') ? 'ON    ' : 'plain  '))
          + (c.textContent || '').trim().slice(0, 12).padEnd(13)
          + 'bg=' + st.backgroundColor.slice(0, 22)
          + ' bc=' + st.borderColor.slice(0, 26);
      });
      // PHASE 2: shift the chip row 120px WITHOUT moving the mouse -> does :hover follow live geometry?
      const host = document.getElementById('f-cats');
      let h2 = ['(no f-cats)'], verdict = 'SKIPPED';
      if (host) {
        host.style.transform = 'translateX(-120px)';
        void host.offsetHeight;  // force layout immediately
        h2 = hoverLabels();
        if (h2.length === 0) verdict = 'RE-SYNCED - hover cleared when chips moved away';
        else if (JSON.stringify(h2) !== JSON.stringify(h1)) verdict = 'RE-SYNCED - hover moved to live geometry (' + h2.join(',') + ')';
        else verdict = 'STALE - hover stuck on ' + h1.join(',') + ' after chips shifted 120px';
        host.style.transform = '';  // revert
      }
      qaOverlay([
        'PHANTOM QA (drawer opened, mouse NEVER moved)',
        'chips total   : ' + chips.length + '   .on=' + onCount + '   :hover(phase1)=' + h1.length,
        'pointer chain : ' + (chain.length ? chain.slice(-6).join(' > ') : 'EMPTY (pointer outside webview)'),
        'phase1 hover  : ' + (h1.length ? h1.join(', ') : 'NONE'),
        'phase2 @-120px: ' + (h2.length ? h2.join(', ') : 'NONE'),
        'VERDICT       : ' + verdict,
        '--- per chip (first ' + Math.min(14, chips.length) + ') ---',
        ...lines,
      ].join('\n'));
    }, 900);
  } catch (e) { qaOverlay('PHANTOM QA ERROR: ' + e); }
}

// QA: prove resize self-heal (cells must rebuild with NO click)
function resizeSelfTest() {
  try {
    const cal = document.getElementById('cal');
    if (!cal) { qaOverlay('RESIZE QA ERROR: no #cal'); return; }
    const c0 = cal.querySelector('.cal-cell[data-date]');
    const r0 = c0 ? c0.getBoundingClientRect() : null;
    const size0 = r0 ? Math.round(r0.width) + 'x' + Math.round(r0.height) : '-';
    // simulate the stale/broken state resize leaves behind
    cal.innerHTML = '<div class="corrupt-probe">CORRUPT</div>';
    // the real resize event -> debounced listener -> renderCalendar()
    window.dispatchEvent(new Event('resize'));
    setTimeout(() => {
      const restored = !!cal.querySelector('.cal-cell[data-date]') && !cal.querySelector('.corrupt-probe');
      const c1 = cal.querySelector('.cal-cell[data-date]');
      const r1 = c1 ? c1.getBoundingClientRect() : null;
      qaOverlay([
        'RESIZE QA (self-heal, no click)',
        'grid before     : ' + size0,
        'corrupted       : YES (simulated stale state)',
        'resize event    : dispatched -> debounced listener',
        'grid restored   : ' + (restored ? 'YES - renderCalendar ran automatically' : 'FAIL - still corrupted'),
        'cell after      : ' + (r1 ? Math.round(r1.width) + 'x' + Math.round(r1.height) + '  ratio ' + (r1.width / r1.height).toFixed(3) : '-'),
        'RESULT          : ' + (restored ? 'OK - cells self-heal on resize' : 'FAIL'),
      ].join('\n'));
    }, 400);
  } catch (e) { qaOverlay('RESIZE QA ERROR: ' + e); }
}

// QA: prove adaptive fill + divider removal with numbers
function cellQa() {
  try {
    const cal = document.getElementById('cal');
    const summary = document.getElementById('cal-dl-summary');
    const head = document.querySelector('.wk-head');
    const list = document.getElementById('wk-list');
    const firstRow = list && (list.querySelector('.dl-item') || list.querySelector('.empty'));
    const cells = cal ? Array.from(cal.querySelectorAll('.cal-cell')) : [];
    const calR = cal ? cal.getBoundingClientRect() : null;
    const rects = cells.map(c => c.getBoundingClientRect());
    const minL = rects.length ? Math.min.apply(null, rects.map(r => r.left)) : 0;
    const maxR = rects.length ? Math.max.apply(null, rects.map(r => r.right)) : 0;
    const c0 = cells.find(c => !c.classList.contains('pad')) || cells[0];
    const cr = c0 ? c0.getBoundingClientRect() : null;
    const cs = c0 ? getComputedStyle(c0) : null;
    const sumCS = summary ? getComputedStyle(summary) : null;
    const gapTL = (head && firstRow) ? Math.round(firstRow.getBoundingClientRect().top - head.getBoundingClientRect().bottom) : -1;
    const row = list && list.querySelector('.dl-item');
    const rowB = row ? (getComputedStyle(row).borderTopWidth + '/' + getComputedStyle(row).borderBottomWidth) : '-';
    const dotCell = cells.find(c => c.querySelector('.dot'));
    let dotInfo = 'no deadline cell';
    if (dotCell) {
      const d = dotCell.querySelector('.dot').getBoundingClientRect();
      const cc = dotCell.getBoundingClientRect();
      dotInfo = 'center off ' + Math.abs(Math.round((d.left + d.width / 2) - (cc.left + cc.width / 2))) + 'px, bottom ' + Math.round(cc.bottom - d.bottom) + 'px';
    }
    qaOverlay([
      'CELL QA  (adaptive fill + divider)',
      'cal width       : ' + (calR ? Math.round(calR.width) : '-') + 'px',
      'RIGHT GAP       : ' + (calR ? Math.round(calR.right - maxR) : '-') + 'px   <- must be ~0 (no empty space)',
      'cell 1st        : ' + (cr ? Math.round(cr.width) + 'x' + Math.round(cr.height) + '  ratio ' + (cr.width / cr.height).toFixed(3) : '-') + '   <- 1.000 = square',
      'font / col-gap  : ' + (cs ? cs.fontSize : '-') + ' / ' + (cal ? getComputedStyle(cal).columnGap : '-'),
      'summary border  : ' + (sumCS ? sumCS.borderTopWidth + ' ' + sumCS.borderTopStyle : '-') + '  <- must be 0px none',
      'title->list gap : ' + gapTL + 'px   (want 8-12, no line)',
      'row borders     : ' + rowB + '   (want 0px/0px = no dividers)',
      'dot centering   : ' + dotInfo,
    ].join('\n'));
  } catch (e) { qaOverlay('CELL QA ERROR: ' + e); }
}

// QA: 12-dummy project across THIS week -> prove weekly panel fixed-height + internal scroll
async function wkScrollTest() {
  try {
    const list0 = await window.api.projects();
    const have = (list0 || []).find(p => p.name === 'ZZ scroll test');
    if (!have) {
      const today = new Date(TODAY);
      const sunday = new Date(today); sunday.setDate(today.getDate() - today.getDay());
      const tasks = [];
      for (let i = 0; i < 12; i++) {
        const d = new Date(sunday); d.setDate(sunday.getDate() + (i % 7));
        const ds = iso(d);
        const hh = String(8 + (i % 10)).padStart(2, '0');
        tasks.push({ text: `scroll dummy ${i + 1}`, deadline: (i % 2 ? `${ds}T${hh}:00` : ds), done: false, url: '' });
      }
      await window.api.saveProject({
        id: null, name: 'ZZ scroll test', website: '', x_account: '',
        category: [], network: [], note: '', icon: '', done: false,
        rank: '', cost: '', status: '', platform: [], wallet_app: '',
        links: [], wallets: [], x_accounts: [], emails: [], tasks,
      });
    ...
    await refresh();
    renderCalendar();
    setTimeout(wkScrollOverlay, 700);
  } catch (e) { toast('wkscroll failed: ' + e, true); }
}
function qaOverlay(text) {
  document.querySelectorAll('.qa-ov').forEach(n => n.remove());
  const ov = document.createElement('div');
  ov.className = 'qa-ov';
  ov.style.cssText = 'position:fixed;top:64px;left:22px;z-index:999;background:#000;color:#3f6;font:12px/1.5 monospace;padding:10px 14px;max-width:780px;white-space:pre-wrap;border:2px solid #3f6;';
  ov.textContent = text;
  document.body.appendChild(ov);
}
function wkScrollOverlay() {
  const list = document.getElementById('wk-list');
  const sum = document.getElementById('cal-dl-summary');
  const main = document.querySelector('.cal-main');
  const cs = list ? getComputedStyle(list) : null;
  const inHead = document.querySelector('.wk-head #cal-toggle');
  qaOverlay([
    'WK SCROLL QA',
    'rows          : ' + (list ? list.querySelectorAll('.dl-item').length : -1),
    'overflow-y    : ' + (cs ? cs.overflowY : 'NO LIST'),
    'scroll/client : ' + (list ? list.scrollHeight + '/' + list.clientHeight + 'px' : '-'),
    'internal scroll: ' + (list && list.scrollHeight > list.clientHeight + 2 ? 'OK' : 'NO'),
    'wk max-height : ' + (sum ? (sum.style.maxHeight || getComputedStyle(sum).maxHeight) : '-'),
    'cal-main h    : ' + (main ? main.offsetHeight + 'px' : '-'),
...
function wkCollapseTest() {
  ...
  if (!btn) { toast('QA: no toggle', true); return; }
  btn.click();
  setTimeout(() => {
    const strip = document.getElementById('cal-strip');
    const gridH = getComputedStyle(document.getElementById('cal-grid-wrap')).display === 'none';
    const listH = getComputedStyle(document.getElementById('wk-list')).display === 'none';
    const both = gridH && listH;
    qaOverlay([
      'WK COLLAPSE QA (master toggle in right column)',
      'collapsed class: ' + (strip.classList.contains('collapsed') ? 'ON' : 'OFF'),
      'calendar grid hidden: ' + (gridH ? 'YES' : 'NO'),
      'weekly list hidden  : ' + (listH ? 'YES' : 'NO'),
      'SYNCED              : ' + (both ? 'OK - both collapsed together' : 'FAIL'),
      'headers visible     : ' + (document.querySelector('.cal-head') && document.querySelector('.wk-head') ? 'YES (month row + weekly title)' : 'NO'),
    ].join('\n'));
  }, 400);
}
async function wkScrollDelete() {
  try {
    const list = await window.api.projects();
    const hits = (list || []).filter(p => p.name === 'ZZ scroll test');
    for (const p of hits) await window.api.deleteProject(p.id);
    await refresh();
    renderCalendar();
    toast('ZZ scroll test deleted: ' + hits.length);
  } catch (e) { toast('wk delete failed: ' + e, true); }
}

// QA: prove WHERE ink sits vs the centered line box (base vs aurora font tokens)
function fontMetric() {
  const el = [...document.querySelectorAll('.tag')].find(t => /Ongoing/.test(t.textContent)) || document.querySelector('.tag');
  if (!el) return;
  const cs = getComputedStyle(el);
  const cv = document.createElement('canvas').getContext('2d');
  const lines = ['FONT METRIC (skin=' + document.documentElement.dataset.skin + ')'];
  lines.push('computed font: ' + cs.fontFamily.split(',')[0]);
  lines.push('box h=' + cs.height + ' pad=' + cs.paddingTop + '/' + cs.paddingBottom + ' lh=' + cs.lineHeight);
  for (const fam of ['Geist', 'Inter', 'system-ui']) {
    cv.font = '11.5px ' + JSON.stringify(fam === 'system-ui' ? fam : fam);
    const loaded = document.fonts.check('11.5px "' + fam + '"');
    const m = cv.measureText('Ongoing');
    // offset: ink center relative to content-area center (>0 = ink LOW)
    const off = ((m.actualBoundingBoxDescent - m.actualBoundingBoxAscent) - (m.fontBoundingBoxDescent - m.fontBoundingBoxAscent)) / 2;
    lines.push(fam + ': loaded=' + loaded + ' off=' + off.toFixed(2) + 'px' + (off > 0 ? ' (LOW)' : off < 0 ? ' (HIGH)' : ''));
  }
  const ov = document.createElement('div');
  ov.style.cssText = 'position:fixed;left:8px;top:52px;z-index:99999;background:#000;color:#39ff14;border:2px solid #39ff14;padding:8px 10px;font:700 13px monospace;white-space:pre;line-height:1.55;';
...
// QA: inspect computed styles of a sample tag (user-mandated BEFORE any fix)
function inspectTag() {
  const el = [...document.querySelectorAll('.tag')].find(t => /Ongoing/.test(t.textContent)) || document.querySelector('.tag');
  if (!el) { return; }
  const cs = getComputedStyle(el);
  ...
  probe.style.cssText = 'font:inherit;';
  el.appendChild(probe);
  const prs = getComputedStyle(probe);
  el.removeChild(probe);
  const lines = [
    'TAG INSPECT (sample: ' + (el.textContent || '').trim() + ')',
    'container display : ' + cs.display,
    'container height  : ' + cs.height,
    'padding-top/bottom: ' + cs.paddingTop + ' / ' + cs.paddingBottom,
    'border-top/bottom : ' + cs.borderTopWidth + ' / ' + cs.borderBottomWidth,
    'box-sizing        : ' + cs.boxSizing,
    'font-size         : ' + cs.fontSize,
    'line-height       : ' + cs.lineHeight,
    'text vert-align   : ' + prs.verticalAlign,
    'parent display    : ' + pcs.display + ' (.' + parent.className + ')',
    'align/justify     : ' + cs.alignItems + ' / ' + cs.justifyContent,
  ];
  const ov = document.createElement('div');
  ov.style.cssText = 'position:fixed;left:8px;top:52px;z-index:99999;background:#000;color:#39ff14;border:2px solid #39ff14;padding:8px 10px;font:700 13px monospace;white-space:pre;line-height:1.55;';
  ov.textContent = lines.join('\n');
...
// QA: numerically verify text centering inside every .tag (same math as devtools)
function measureTagCentering() {
  const rows = [];
  document.querySelectorAll('.tag').forEach(el => {
    ...
    const topGap = (tr.top - r.top) - bt;          // border-box top -> text top
    const botGap = (r.bottom - bb) - tr.bottom;     // border-box bottom -> text bottom
    const ok = Math.abs(topGap - botGap) <= 0.6;
    rows.push(`${(el.textContent || '').trim().slice(0, 13).padEnd(14)} T=${topGap.toFixed(2)} B=${botGap.toFixed(2)} ${ok ? 'OK' : 'OFF'}`);
  });
  const ov = document.createElement('div');
  ov.id = 'tagmeasure-ov';
  ov.style.cssText = 'position:fixed;left:8px;top:52px;z-index:99999;background:#000;color:#39ff14;border:2px solid #39ff14;padding:8px 10px;font:700 13px monospace;white-space:pre;line-height:1.5;';
  ov.textContent = 'TAG CENTER MEASURE (T vs B must match)\n' + rows.join('\n');
  document.body.appendChild(ov);
}
